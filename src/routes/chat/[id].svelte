...
<div class="m-4 left-0 p-8 md:m-8">
	<a
		href="/login"
		class="text-center text-lg font-normal py-2 px-4 md:py-4 rounded-lg text-white bg-orange-400"
		>Back</a
	>
</div>
		...
		<div class="mx-auto bg-gray-50 drop-shadow-md rounded-md shadow-sm p-6 w-full h-auto">
			<div class="flex flex-wrap space-x-4 space-y-4 justify-between items-center">
				<div class="flex justify-start items-center">
					<div class="mr-4">
						<img
							src="https://avatars.dicebear.com/api/initials/{$USER
								? $USER.fullname
								: 'Anonymous'}.svg"
					...
					<div>
						<h4 class="text-gray-600 font-medium text-md">Chat NOW!!</h4>
				...
				{#if booking.is_paid == 'no'}
					<div>
						{#if $USER && $USER.type == 'user' && booking.checkout == 'yes'}
							<a
								target="_blank"
								href="https://happivibes-backend.com/api/pay/{chat_id}"
								class="text-center text-sm md:text-md font-normal py-2 px-4 rounded-lg text-white bg-orange-400"
								>PAY WITH CARD</a
							>
							<a
								target="_blank"
								href="https://happivibes-backend.com/api/pay-wallet/{chat_id}"
						...
						{#if $USER && $USER.type == 'agent'}
							<span
								on:click={PAYMENT(booking.order_id, booking.checkout)}
			...
			<div class="flex justify-center items-center">
				<div
					class="mx-2 lg:mx-4 bg-orange-100 p-4 rounded-lg my-4 text-gray-600 font-normal text-sm"
				>
					Messages are end-to-end encrypted. No one outside of this chat, can read or listen to
					them.
				</div>
			</div>
			...
			<div class="flex justify-start items-center bg-gray-200">
				...
				<div>
					<button
						type="button"
						on:click|preventDefault={SEND_MESSAGE}
						class="bg-orange-400 text-white text-sm font-bold p-4 h-[40px] rounded-r-lg"
						>SEND</button
					>
				</div>
			</div>
